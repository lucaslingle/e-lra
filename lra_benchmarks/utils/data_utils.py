...
def build_vocab(train_dataset, fields, vocab_size=None, min_freq=0):
    ...
    logging.info(
        "Vocab size: before filtering (%d), after(%d)", len(word_freqs), len(vocab)
    )
    ...
    with tempfile.NamedTemporaryFile(delete=False, prefix="/tmp/ds_chars") as outfp:
        while char_count < maxchars:
            ...
            for k in data_keys:
                ...
                outfp.write(line)
    logging.info(
        "%d examples processed for training sentencepiece tokenizer.",
        processed_examples,
    )
...
def load_sentencepiece_tokenizer(model_path, add_bos=True, add_eos=True, reverse=False):
