...
"""Main script for document retrieval in dual encoder style."""
import functools
import itertools
import json
import os
...
from lra_benchmarks.retrieval import input_pipeline
...
flags.DEFINE_string(
    "task_name", default="retrieval", help="Directory to store model data."
)
    ...
    def _create_model(key):
        ...
        with nn.stochastic(key):
            _, initial_params = module.init_by_shape(
                key, [(input1_shape, jnp.float32), (input2_shape, jnp.float32)]
            )
...
def main(argv):
    ...
    model = train_utils.get_model(
        model_type, create_model, model_kwargs, init_rng, input_shape, input_shape
    )
