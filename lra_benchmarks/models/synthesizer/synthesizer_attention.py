    ...
    def apply(
        ...
        if "random" in synthesizer_mode:
            if "factorized_random" in synthesizer_mode:
                ...
                rand_syn_weights1 = self.param(
                    "random1", (num_heads, max_length, k), kernel_init
                )
                rand_syn_weights2 = self.param(
                    "random2", (num_heads, k, max_length), kernel_init
                )
                rand_syn_weights1 = rand_syn_weights1[:, :qlength, :]
                rand_syn_weights2 = rand_syn_weights2[:, :, :kvlength]
                rand_syn_weights = jnp.einsum(
                    "hlk,hkn->hln", rand_syn_weights1, rand_syn_weights2
                )
                rand_syn_weights = jax.lax.broadcast(
                    rand_syn_weights, (inputs_q.shape[0],)
                )
        ...
        if "dense" in synthesizer_mode:
            dense_syn = nn.DenseGeneral.partial(
                axis=-1,
                features=(num_heads, head_dim),
                kernel_init=kernel_init,
                bias_init=bias_init,
