...
class SinkhornAttention(nn.Module):
    ...
    def apply(
        ...
        if sort_activation == "sinkhorn":
            ...
            permutation = jnp.reshape(
                permutation, (-1, num_kv_blocks, num_heads, num_query_blocks)
            )
        else:
            if causal_mask:
        ...
        if padding_mask is not None:
            ...
            padding_mask = jnp.reshape(
                padding_mask, (bs * num_query_blocks, block_size, 1)
            )
            ...
            padding_mask = jnp.reshape(
                padding_mask, (bs, num_query_blocks, block_size, block_size)
            )
            mask_components.append(padding_mask)
            sorted_padding_mask = jnp.einsum(
                "bksj,bnhl->bnsj", padding_mask, permutation
            )
        ...
        if segmentation is not None:
            ...
            segmentation_mask = jnp.reshape(
                segmentation_mask, (bs, num_query_blocks, block_size, block_size)
            )
            mask_components.append(segmentation_mask)
            sorted_segmentation_mask = jnp.einsum(
                "bksj,bnhl->bnsj", segmentation_mask, permutation
            )
