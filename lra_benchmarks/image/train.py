...
def test(optimizer, state, p_eval_step, step, test_ds, summary_writer, model_dir):
    ...
    for _, test_batch in zip(itertools.repeat(1), test_iter):
        ...
        test_batch = common_utils.shard(jax.tree_map(lambda x: x._numpy(), test_batch))
...
def train_loop(
    ...
    for step, batch in zip(range(start_step, num_train_steps), train_iter):
        ...
        if step % eval_freq == 0 and step > 0:
            ...
            logging.info(
                "train in step: %d, loss: %.4f, acc: %.4f",
                step,
                summary["loss"],
                summary["accuracy"],
            )
            if jax.process_index() == 0:
                ...
                summary_writer.scalar(
                    "examples_per_second", steps_per_sec * config.batch_size, step
                )
...
def main(argv):
    ...
    (
        ...
        vocab_size,
        input_shape,
    ) = task_registry.TASK_DATA_DICT[FLAGS.task_name](
        n_devices=jax.local_device_count(), batch_size=batch_size, normalize=normalize
    )
    ...
    model, state = train_utils.get_model(
